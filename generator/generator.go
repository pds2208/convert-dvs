...
func GeneratePreamble(program *ast.Program, name string) string {
...
func Eval(node ast.Node) string {
	...
	case *ast.ArrayStatement:
		return evalArrayStatement(node)
...
func evalArrayStatement(node *ast.ArrayStatement) string {
	var result bytes.Buffer
	result.WriteString(node.Name.String() + "= []\n")
	return result.String()
}
...
func evalBlockStatement(block *ast.BlockStatement) string {
	var result bytes.Buffer
